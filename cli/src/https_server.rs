...
pub fn run<T: pontus_onyx::Engine + Send + 'static>(
	...
	let port = program_state.lock().unwrap().https_port.clone();

	match port {
		Some(port) => {
			match &settings.https {
				Some(settings_https) => {
					match std::fs::File::open(&settings_https.keyfile_path) {
						Ok(keyfile_content) => {
							match std::fs::File::open(&settings_https.certfile_path) {
								Ok(cert_content) => {
									...
									match rustls_pemfile::certs(cert_file) {
										Ok(cert_chain) => {
											match rustls_pemfile::pkcs8_private_keys(key_file) {
													...
													match keys.get(0) {
														Some(key) => {
															...
															match server_config {
																...
																Err(e) => {
																	Err(format!("can not insert certificate in server : {e}"))
																}
															}
														}
														None => Err(format!(
												...
												Err(e) => Err(format!(
													"can not read PKCS8 private key : {e}"
												)),
											}
										}
										Err(e) => {
											Err(format!("can not read SSL certificate : {e}"))
										}
									}
								}
								Err(e) => Err(format!(
