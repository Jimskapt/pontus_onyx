...
fn multiple_modules() {
	...
	assert_eq!(
		token.check_request(&crate::Request::get("folder_a/").unwrap()),
		Ok(())
	);
	assert_eq!(
		token.check_request(&crate::Request::get("folder_b/").unwrap()),
		Ok(())
	);
	assert_eq!(
		token.check_request(&crate::Request::get("folder_c/").unwrap()),
		Err(vec![
...
fn check_request() {
	...
	assert_eq!(
		bearer.check_request(&crate::Request::get("folder_a/").unwrap()),
		Ok(())
	);
	...
	assert_eq!(
		bearer.check_request(&crate::Request::get("folder_b/").unwrap()),
		Err(RequestValidityError::OutOfModuleScope)
	);
